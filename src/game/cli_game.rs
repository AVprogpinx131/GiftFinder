...
pub fn choose_calendar_date() -> Result<i32, Box<dyn Error>> {
    ...
    let salary_with_bonus = match birthday_weekday {
        Weekday::Sat | Weekday::Sun => {
            println!("It's a weekend. Adding bonus to salary.");
            base_salary + random_number
        }
        _ => base_salary,
    };
...
pub fn choose_birthday_item(mut salary: i32) {
    let mut gifts: HashMap<&str, i32> = HashMap::from([
        ("laptop", 1000),
        ("smartphone", 500),
        ("tablet", 300),
        ("headphones", 200),
    ...
    loop {
        ...
        let selected_gift = Select::new("Choose a birthday gift:", gift_options)
            .prompt()
            .unwrap();
        ...
        if salary < gift_price {
            println!(
                "You don't have enough money to buy {} which costs {}$",
                selected_gift, gift_price
            );
            continue;
        }
        ...
        if salary > 0 {
            ...
            let continue_shopping =
                Select::new("Do you want to buy another gift?", vec!["Yes", "No"])
                    .prompt()
                    .unwrap();
